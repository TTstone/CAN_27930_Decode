...
with open(input_path, 'rt',encoding='utf-8') as f:
    data = f.read()
out_SPN = input_path+"_SPN_time_sinceCHM.csv"
out_path = input_path+"_output.csv"
...
with open(out_path, 'w') as f:
    ...
    f.write("\n")
    i = -1
    j_1 = data.rfind("181356f4x BSM_BCU") 
    j_2 = data.rfind("cts                              0 0 8  8 11 02 01 ff ff 00 11 00")
    j = min(j_1,j_2)
    
    while i < j:
        start = i + 1
        i_1 = data.index('cts                              0 0 8  8 11 02 01 ff ff 00 11 00', start)
        i_2 = data.index('181356f4x BSM_BCU', start)
        i = min(i_1,i_2)
...
print("报文第一次出现时间以及充电记录文件已经保存在原始文件目录")
